...
export default function ToggleButton_Notice(props) {
  ...
  useEffect(() => {});
  const setNoticeShow = async (noticeno, show) => {
    try {
      await axios.post(
        `${"http://localhost:2500"}/admin/notice/show`,
        { noticeno, show },
        {
          headers: {
  ...
  const handleToggle = () => {
    ...
    setNoticeShow(+noticeno, Number(!isActive));
    onClick();
  };
  return (
    <Toggle
